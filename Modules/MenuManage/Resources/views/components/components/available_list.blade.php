                ...
                <div class="dd-list min-height-400 available-items-container" data-id="remove" data-section_id="remove" id="available_list">
                    ...
                    @if($unused_menus->count())
                        @foreach($unused_menus as $menu)
                            {{-- @if(!$menu->permissionInfo->module or isModuleActive($menu->permissionInfo->module))
                                @if(permissionCheck($menu->permissionInfo->route)) --}}
                                
                                    <div class="dd-item listed_menu" data-id="{{$menu->id}}" data-parent_id= "{{$menu->parent_id}}" data-route= "{{$menu->parent_id}}">
                                        <div class="dd-handle">
                                            <div class="menu_icon">
